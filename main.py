...
# for some reason I can't make it global without it breaking the function
        if computer_health <= 0:
            ...
            print("You " + currentattack + "ed for " + str(atck_dict[currentattack]['Damage'])
                  + " damage and their health is now " + str(computer_health))
    else:
        print("You missed!")
...
# Randomly picks an attack for the computer to attempt, all attacks are given equal weighting
    ...
    currentattack = list(atck_dict.keys())[roll]
    dice(6)
    if roll >= atck_dict[currentattack]['Chance']:
        player_health -= atck_dict[currentattack]['Damage']
        if player_health <= 0:
            print("They knocked you out!")
        else:
            print("They " + currentattack + "ed you for " + str(atck_dict[currentattack]['Damage']) +
                  " damage and your health is now " + str(player_health))
    else:
        print("They tried to " + currentattack + " you but missed!")
